...
/*
        for (ChessPiece anotherPiece : pieces) {
            if (piece.getCurrentPosition().equals(piece.getCurrentPosition())) {
                return false;
            }
        }
*/
    ...
    public Set<DeskCell> removeNotAllowed(Set<DeskCell> cells, boolean team) {
        ...
        while(iter.hasNext()) {
            ...
            for (ChessPiece piece : pieces) {
                if (piece.getCurrentPosition().equals(cell)) {
...
*/
    ...
    public boolean move(ChessPiece piece, DeskCell dest) {
        ...
        while(iter.hasNext()) {
            ...
            if (anotherPiece.getCurrentPosition().equals(dest)) {
...
/*
  ...
  */      
    ...
    public List<DeskCell> getBusyCells() {
        ...
        for (ChessPiece piece : pieces) {
            result.add(piece.getCurrentPosition());
        }
        return result;
    }

    public boolean checkShah(boolean kingTeam) {
        //System.err.println("Checking shah to " + kingTeam + " team");

        List<DeskCell> busyCells = new ArrayList<>();
        King king = kingTeam ? whiteKing : blackKing;

        for (ChessPiece piece : pieces) {
            if (piece.getTeam() != kingTeam && 
                    piece.getAvailableMoves(lowerLimit, upperLimit, busyCells).contains(king.getCurrentPosition())) {
                /*
                System.err.println("Shah by " + piece + " [" + piece.getCurrentPosition().getX() + "|" + piece.getCurrentPosition().getY() + "] . It's available moves:");
                for (DeskCell moving : piece.getAvailableMoves(lowerLimit, upperLimit, busyCells)) {
                    System.err.println(moving.getX() + " " + moving.getY());
                }*/

                return true;
            }
        }
        System.err.println("No shah");        
